...
class MyRenderer: CustomRenderer {
    ...
    func updateResources() {
        ...
        let angleDegrees = 360 * (numSeconds / 2)
        
        let angleRadians = degreesToRadians(angleDegrees)
        let zAxis: simd_float3 = [0.0, 0.0, 1.0]
        let timeRotation = simd_quatf(angle: angleRadians, axis: zAxis)
        
        let cameraRotation = simd_quatf(cameraToWorldTransform)
        let orientation = cameraRotation * timeRotation
        
        let cameraDirection = cameraRotation.act(-zAxis)
        let headPosition = interfaceCenter
        
        let arrowDistance: Float = 0.6
        let arrowCenter = headPosition + arrowDistance * cameraDirection
        
        drawArrow(center: arrowCenter, orientation: orientation)
    }
