...
ARHeadsetKit brings more than just AR experiences. With easy-to-render AR objects and a CPU ray tracing API, ARHeadsetKit provides a unique environment for experimenting with 3D graphics. Its tutorial series simultaneously serve as an introduction to Swift and iOS, using AR to make the learning process interactive. It can even be used for creating VR apps using a high-level programming language.
